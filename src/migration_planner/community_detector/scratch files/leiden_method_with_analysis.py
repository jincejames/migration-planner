...
stream_stream_dependency_df.coalesce(1).write.mode("overwrite").option("header", "true").csv(output_path +  "stream_stream_dependencies_output.csv")
...
# # If "to" contains "json", set table_weight = 2 regardless of size, this is to give slightly high preference to BI reports
...
# ---- CONFIG ----
SCALED_MIN = 0.0  # <ToDo> Ensure min is never 0
SCALED_MAX = 1.0
...
df = df.withColumn(
    "raw_table_weight",
    when(col("raw_table_weight").isNull() | (col("raw_table_weight") <= 0), lit(SCALED_MIN))
...
resolutions = [0.4, 0.5, 0.6, 0.8, 1.0, 1.2, 1.5, 1.8, 2.2, 2.6]
...
def plot_communities_with_analysis_safe(
    ...
    max_labels=120,
    enable_plotting=False,  # NEW: Plotting disabled by default
):
    import os
    import numpy as np
    import networkx as nx
...
for res in resolution:
    ...
    saved = plot_communities_with_analysis_safe(
        ...
        max_labels=120,
        enable_plotting=False  # Plotting disabled by default
    )
...
#     def __init__(self, stream_table_dependency_df, merged_edges_df, leiden_df, size_weight=0.75, count_weight=0.2.65):
#         """
...
#     # Save figure
#     viz_file = os.path.join(output_path, f"community_ordering_visualization_gamma_{res}.png")
