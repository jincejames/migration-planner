...
current_dependency_df_full = spark.read.format("csv").option("header", "true").option("delimiter", ";").load("/Volumes/odp_adw_mvp_n/migration/utilities/community_detection/ETL-table-dependencies_11022026.csv")

# COMMAND ----------

# DBTITLE 1,Analysis: See previous sources and targets of IMP_JV_RSE stream
old_imp_jv_rse_filtered = old_dependency_df_full.filter(
    (col("STREAM_NAME") == "IMP_JV_RSE") & (upper(col("table_type")).isin(["TGT", "SRC"]))
).select("SquadID", "STREAM_NAME", "DB_Table_Name", "table_type").distinct()
...
old_imp_jv_rse_with_size = old_imp_jv_rse_filtered.join(
    table_size_df,
    upper(old_imp_jv_rse_filtered.DB_Table_Name) == table_size_df.table_name,
    "left"
).select(
...
current_imp_jv_rse_filtered = current_dependency_df_full.filter(
    (col("STREAM_NAME") == "IMP_JV_RSE") & (upper(col("table_type")).isin(["TGT", "SRC"]))
).select("SquadID", "STREAM_NAME", "DB_Table_Name", "table_type").distinct()

current_imp_jv_rse_with_size = current_imp_jv_rse_filtered.join(
    table_size_df,
    upper(current_imp_jv_rse_filtered.DB_Table_Name) == table_size_df.table_name,
    "left"
).select(
...
old_imp_jv_rse_sources = old_imp_jv_rse_filtered.filter(upper(col("table_type")) == "SRC").select("DB_Table_Name").distinct()
current_imp_jv_rse_sources = current_imp_jv_rse_filtered.filter(upper(col("table_type")) == "SRC").select("DB_Table_Name").distinct()

# Find streams that produce these tables as targets (upstream dependencies) - OLD data
old_upstream_deps = old_dependency_df_full.alias("dep").join(
    old_imp_jv_rse_sources.alias("src"),
    upper(col("dep.DB_Table_Name")) == upper(col("src.DB_Table_Name")),
    "inner"
).filter(
    (upper(col("dep.table_type")) == "TGT") & (col("dep.STREAM_NAME") != "IMP_JV_RSE")
).select(
    col("dep.SquadID"),
    col("dep.STREAM_NAME"),
...
current_upstream_deps = current_dependency_df_full.alias("dep").join(
...
old_imp_jv_rse_targets = old_imp_jv_rse_filtered.filter(upper(col("table_type")) == "TGT").select("DB_Table_Name").distinct()
current_imp_jv_rse_targets = current_imp_jv_rse_filtered.filter(upper(col("table_type")) == "TGT").select("DB_Table_Name").distinct()

# Find streams that use these tables as sources (downstream dependencies) - OLD data
old_downstream_deps = old_dependency_df_full.alias("dep").join(
    old_imp_jv_rse_targets.alias("tgt"),
    upper(col("dep.DB_Table_Name")) == upper(col("tgt.DB_Table_Name")),
    "inner"
).filter(
    (upper(col("dep.table_type")) == "SRC") & (col("dep.STREAM_NAME") != "IMP_JV_RSE")
).select(
    col("dep.SquadID"),
    col("dep.STREAM_NAME"),
    col("dep.DB_Table_Name"),
...
current_downstream_deps = current_dependency_df_full.alias("dep").join(
...
print(f"Total unique dependencies: {old_all_deps.select('STREAM_NAME', 'DB_Table_Name', 'dependency_type').distinct().count()}")
...
print(f"Total unique dependencies: {current_all_deps.select('STREAM_NAME', 'DB_Table_Name', 'dependency_type').distinct().count()}")
print(f"\nBreakdown by dependency type:")
...
current_dependency_df = current_dependency_df_full.select("SquadID","STREAM_NAME","STREAM_TYPE","DB_Table_Name","table_type", "script_name").distinct()
...
tgt_tables_df = report_current_dependency_df_full.filter(
    upper(col("table_type")) == "TGT"
).select("SquadID", "STREAM_NAME", "STREAM_TYPE","script_name", "DB_Table_Name").distinct()

src_tables_df = report_current_dependency_df_full.filter(
    upper(col("table_type")).isin("SRC", "SRC_TRNS")
).select("DB_Table_Name").distinct()

tgt_only_tables_df = tgt_tables_df.alias("tgt").join(
    src_tables_df.alias("src"),
    upper(col("tgt.DB_Table_Name")) == upper(col("src.DB_Table_Name")),
    "left_anti"
).select(col("tgt.*")).distinct()
...
tgt_only_tables_df = tgt_only_tables_df.alias("tgt").join(
...
all_tgt_by_stream = report_current_dependency_df_full.filter(
    upper(col("table_type")) == "TGT"
).select("SquadID", "STREAM_NAME", "STREAM_TYPE", "DB_Table_Name").distinct()
...
tgt_with_usage = all_tgt_by_stream.alias("tgt").join(
    unused_tables.alias("unused"),
    upper(col("tgt.DB_Table_Name")) == upper(col("unused.DB_Table_Name")),
    "left"
).withColumn(
    "is_unused",
    when(col("unused.DB_Table_Name").isNotNull(), lit(1)).otherwise(lit(0))
).select(
    col("tgt.*"),
    col("is_unused")
)
...
stream_target_stats = tgt_with_usage.groupBy("SquadID", "STREAM_NAME", "STREAM_TYPE").agg(
    count("*").alias("total_targets"),
    sum("is_unused").alias("unused_targets")
)

# Find streams where ALL targets are unused (total_targets == unused_targets)
streams_only_unused = stream_target_stats.filter(
    col("total_targets") == col("unused_targets")
).orderBy("SquadID", "STREAM_TYPE", "STREAM_NAME")
...
in_scope_streams_only_unused = streams_only_unused.filter(~col("STREAM_NAME").isin(outofscope_stream_names_list)).orderBy("SquadID", "STREAM_TYPE")
...
stats_df = tgt_only_tables_df_sorted.groupBy("SquadID", "STREAM_TYPE").agg(
    count("*").alias("count")
)

# Count per STREAM_TYPE only
stats_by_streamtype_df = tgt_only_tables_df_sorted.groupBy("STREAM_TYPE").agg(
    count("*").alias("count")
)
...
stats_with_pct = stats_df.withColumn(
    "total", lit(total_tables)
).withColumn(
    "percentage", spark_round((col("count") / col("total")) * 100, 2)
).select(
    "SquadID", "STREAM_TYPE", "count", "total", "percentage"
).orderBy("SquadID", "STREAM_TYPE")

# Add total and percentage columns for STREAM_TYPE only
stats_by_streamtype_with_pct = stats_by_streamtype_df.withColumn(
    "total", lit(total_tables)
).withColumn(
    "percentage", spark_round((col("count") / col("total")) * 100, 2)
).select(
    "STREAM_TYPE", "count", "total", "percentage"
).orderBy("STREAM_TYPE")
